...
export async function POST(request: Request) {
  try {
    ...
    const variations = Math.min(Math.max(body.variations ?? 3, 1), 6);
    const system = appSettings.systemPrompts.designerGenerate;

    const user = [
      `Prompt: ${body.prompt}`,
      body.history ? `Chat-Historie:\n${body.history}` : "",
      `Create ${variations} distinct UI variations.`,
      "Use the Candy Studio design language: dark, cinematic, glassy, modern.",
    ].join("\n");
    ...
    if (!images.length) {
      ...
      const parsed = safeParseJson<DesignerResponse>(response.content);
      if (!parsed || !Array.isArray(parsed.variations)) {
        return NextResponse.json({ error: "Antwort konnte nicht geparst werden." }, { status: 500 });
      }
    ...
    if (body.provider === "openai") {
      ...
      const data = await response.json();
      content = data?.choices?.[0]?.message?.content ?? data?.choices?.[0]?.content ?? "";
    } else {
      return NextResponse.json({ error: "Provider nicht unterstuetzt." }, { status: 400 });
    }

    const parsed = safeParseJson<DesignerResponse>(content);
    if (!parsed || !Array.isArray(parsed.variations)) {
      return NextResponse.json({ error: "Antwort konnte nicht geparst werden." }, { status: 500 });
    }
    ...
    const message = error instanceof Error ? error.message : "Designer request failed.";
